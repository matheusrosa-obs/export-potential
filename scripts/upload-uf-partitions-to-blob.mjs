...
/** @typedef {{ dataset: string; sg_uf: string; file_name: string; size_bytes?: number; rows?: number; blob_url?: string }} IndexEntry */
  ...
  for (let i = 0; i < entries.length; i += concurrency) {
    ...
    const uploadedBatch = await Promise.all(
      batch.map(async (entry) => {
        const filePath = path.join(partitionsDir, entry.dataset, entry.file_name);
        const fileBuffer = await fs.readFile(filePath);
        const blobPath = `ufs/${entry.dataset}/${entry.file_name}`;
